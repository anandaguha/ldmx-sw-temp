...
void saveNumberOfHits (int number, const std::string& filename) {
    ...
    if (outFile.is_open()) { // Check if the file is open
        ...
        if (inFile.tellg() == 0) {
            outFile << number; // If the file is empty, just write the number
        } else {
            outFile << " " << number; // If the file is not empty, append a space and then the number
        }
        inFile.close();
        outFile.close(); // Close the file
        std::cout << "Number " << number << " has been appended to " << filename << std::endl;
    } else {
...
//      std::cout << "Made it into the if clause. THe Zero Supression value is: " << zero_suppression_ << std::endl;
      ...
      //saves size of noiseHitAmplites to a file imdiatley
      saveNumberOfHits(noiseHitAmplitudes.size(),"/home/ananda/ldmx-results/noiseHitCountNormal.csv" );


      std::vector<std::pair<double,double>> fake_pulse(1,{0.,0.});
//      std::cout << " [Number of empty channels]: " << numEmptyChannels <<std::endl;
//      std::cout << " noise Amps: " << noiseHitAmplitudes <<std::endl;
...
//        std::cout << "Number of hits" << noiseHit;
...
//      std::cout << "Made it into the else clause" << std::endl;
